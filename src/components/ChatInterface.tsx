...
function UtilityBar({ content, mode, messageId, userName }: {
  ...
  const handlePdf = () => {
    ...
    const html = [
      ...
      "/* ── Print-safe: override dark-theme CSS variables with light-theme values so all var()-based colors are dark on white paper ── */",
      ":root { --background: 0 0% 100%; --foreground: 222 47% 11%; --card: 0 0% 100%; --card-foreground: 222 47% 11%; --popover: 0 0% 100%; --popover-foreground: 222 47% 11%; --primary: 200 98% 35%; --primary-foreground: 0 0% 100%; --secondary: 210 40% 96%; --secondary-foreground: 222 47% 11%; --muted: 210 40% 96%; --muted-foreground: 215 16% 47%; --accent: 200 98% 35%; --accent-foreground: 0 0% 100%; --border: 214 32% 91%; --input: 214 32% 91%; --ring: 200 98% 35%; --sidebar-background: 0 0% 100%; --sidebar-foreground: 222 47% 11%; --sidebar-border: 214 32% 91%; }",
      "/* Explicit white-text safety net (inline style overrides via remapEl JS handles these too) */",
      "[class*='text-white'] { color: #111827 !important; }",
      "code, code * { color: #1e3a5f !important; background: #dbeafe !important; border-color: #bfdbfe !important; }",
